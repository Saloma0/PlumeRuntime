...
local executionPath = "lib/execution.lua" -- ou use o execution nesse codigo!

if isfile(executionPath) then
    info("carregando arquivo:" .. executionPath)

    local executionChunk, err = loadfile(executionPath)

    if not executionChunk then
        erro("Erro:" .. tostring(err))
    else
        local executionSuccess, result = pcall(executionChunk)

        if not executionSuccess then
            erro("Erro de execucao:" .. tostring(result))
        else
            success("arquivo execution executado com sucesso!")
        end
    end
else
    warn("arquivo de execução não encontrado: " .. executionPath)
end
